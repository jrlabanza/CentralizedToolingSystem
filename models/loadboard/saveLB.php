...
$mb1_check = strtoupper($_POST['mb1']);
$mb2_check = strtoupper($_POST['mb2']);
...
// $result = $conn->query("SELECT dut_board_name, main_board_1, main_board_2 from pizza_dut WHERE main_board_1 = '$mb1_check' AND isDeleted = 0");
$result = $conn->query("SELECT dut_board_name, main_board_1, main_board_2 from pizza_dut WHERE (main_board_1 = '$mb1_check' AND  lb_ID = '$srid') AND isDeleted = 0");
...
$result2 = $conn->query("SELECT dut_board_name from pizza_dut WHERE main_board_2 = '$mb2_check' AND isDeleted = 0");
while ($row2 = $result2->fetch_assoc()) {
    $mb2_res = $row2['dut_board_name'];
}

// $result3 = $conn->query("SELECT dut_board_name from pizza_dut WHERE lb_ID = '$srid' AND isDeleted = 0");
// while ($row3 = $result3->fetch_assoc()) {
//     $sr_validity = $row3['dut_board_name'];
// }

$result4 = $conn->query("SELECT dut_board_name from pizza_dut WHERE main_board_1 = '$mb1_check' AND isDeleted = 0");
while ($row4 = $result4->fetch_assoc()){
    ...
    $resLen = $row4['dut_board_name'];
    // echo "<script type='text/javascript'>alert('$resLen');</script>";
    if ($sr_validity == $resLen){
        $dut_checker = "GOOD";
    }
}
...
// if($conCheck == "OUT-PROD"){
    if($mb1_check != "N/A"){ // check if mainboard 1 has data
        ...
        if($dut_checker != "GOOD"){
            ...
            if($dut_checker != "GOOD"){ // if mainboard 1 is empty, cancel
