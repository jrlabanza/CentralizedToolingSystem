...
if($_POST['id']){
	$id=$_POST['id'];
	$result = $conn->query('SELECT * FROM socket_board WHERE package_type LIKE "%'.$id.'%" OR socket_id LIKE "%'.$id.'%" OR handler_id LIKE "%'.$id.'%" OR package_type LIKE "%'.$id.'%" OR status LIKE "%'.$id.'%" ORDER BY last_update DESC');
