...
if (empty($_SESSION['line'])){
	$sql = "SELECT * from socket_board ORDER BY last_update DESC";
}else{
	$sql = "SELECT * FROM socket_board WHERE line = '$_SESSION[line]' ORDER BY last_update DESC";
}

$paginationlink = "models/socketBoard/pagiResSocketBoard.php?page=";
...
if(!empty($perpageresult)) {
	$output = '<nav aria-label="..."><ul class="pagination">' . $perpageresult . '</ul><button type="button" class="btn btn-link" onclick=location.href="models/socketBoard/exportInventory.php">Download Inventory</button></nav>';
	$output .= '';
}else{
	$output = '';
}
		...
		foreach($faq as $k=>$v) {
			$output .= '
					<tr style="cursor: pointer;">
							<td class="lbID">' . $faq[$k]['socket_id'] . '</td>
							<td class="fam">' . $faq[$k]['family'] . '</td>
							<td class="td1">' . $faq[$k]['temp'] . '</td>
							<td class="pckgType">' . $faq[$k]['package_type'] . '</td>
							<td class="prtNo">' . $faq[$k]['part_no'] . '</td>
							...
							<td class="mShotCnt">' . $faq[$k]['max_shotcount'] . '</td>
