...
if(empty($fileName)){
    ...
    $conn->query("UPDATE spanker set status='".$status."',machine_id='".$machineID."',shots='".$shotCnt."',remarks='".$remarks."',loc='".$loc."' WHERE box_no = '".$boxNo."'");

    $conn->query("INSERT INTO spanker_history (box_no, package, nozzle_partno, altrntv_nozzle, line, machine_model, machine_id, out_count, status, loc, max_shots, shots, remarks, client, clerk) 
            SELECT box_no, package, nozzle_partno, altrntv_nozzle, line, machine_model, machine_id, out_count, status, loc, max_shots, shots, remarks,'$borrower','".$_SESSION['userEmail']."' FROM spanker WHERE box_no = '".$boxNo."'");
