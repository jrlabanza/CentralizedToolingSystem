...
if($_POST['id']){
	$id=$_POST['id'];
	$result = $conn->query('SELECT * FROM test_stand WHERE serial_id LIKE "%'.$id.'%" OR family LIKE "%'.$id.'%" OR 
	status LIKE "%'.$id.'%" OR tst_pf LIKE "%'.$id.'%" OR status LIKE "%'.$id.'%" OR tester_id LIKE "%'.$id.'%" OR 
	handler_id LIKE "%'.$id.'%" OR loc LIKE "%'.$id.'%" OR rack LIKE "%'.$id.'%" OR vendor LIKE "%'.$id.'%" OR 
	line LIKE "%'.$id.'%" OR last_update LIKE "%'.$id.'%" AND line LIKE "%'.$_SESSION['line'].'%"');
