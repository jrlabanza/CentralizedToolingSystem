...
if (empty($_SESSION['line'])){
	$sql = "SELECT * from ic ORDER BY id DESC";
}else{
	$sql = "SELECT * from ic WHERE line = '$_SESSION[line]' ORDER BY id DESC";
}

$paginationlink = "models/editItems/editResIC.php?page=";
	...
	foreach($faq as $k=>$v) {
		$output .= '
				<tr style="cursor: pointer;">
					...
					<td class="td1">' . $faq[$k]['lf_name'] . '</td>
					<td class="td3">' . $faq[$k]['package'] . '</td>
					<td class="td6">' . $faq[$k]['machine_model'] . '</td>
					<td class="">' . $faq[$k]['machine_id'] . '</td>
					<td class="td8">' . $faq[$k]['dra'] . '</td>
					<td class="td4">' . $faq[$k]['insert_sn'] . '</td>
					<td class="td9">' . $faq[$k]['clamp_sn'] . '</td>
					<td class="">' . $faq[$k]['loc'] . '</td>
					<td class="">' . $faq[$k]['out_count'] . '</td>
					<td class="">' . $faq[$k]['status'] . '</td>
					<td class="">' . $faq[$k]['shots'] . '</td>
					<td class="td7">' . $faq[$k]['max_shots'] . '</td>
