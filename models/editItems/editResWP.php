...
if (empty($_SESSION['line'])){
	$sql = "SELECT * from wp ORDER BY last_update DESC";
}else{
	$sql = "SELECT * FROM wp WHERE line = '$_SESSION[line]' ORDER BY last_update DESC";
}

$paginationlink = "models/workpress/pagiResWP.php?page=";
...
$output .= '<table id="keywords" class="table table-bordered table-sm-responsive table-hover">
	<thead id="thead" class="thead-light">
		<tr>
			...
			<th scope="col" class="col-sm-0">DATA LOG</th>
		</tr>
	</thead>
	<tbody class="tbody">';
		...
		foreach($faq as $k=>$v) {
			$output .= '
				<tr style="cursor: pointer;">
					...
					<td class="td11">' . $faq[$k]['material'] . '</td>
					<td class="">' . $faq[$k]['status'] . '</td>
					<td class="td7">' . $faq[$k]['tst_model'] . '</td>
					<td class="">' . $faq[$k]['tester_id'] . '</td>
					<td class="td8">' . $faq[$k]['hd_model'] . '</td>
					<td class="">' . $faq[$k]['handler_id'] . '</td>
					<td class="">' . $faq[$k]['loc'] . '</td>
					<td class="td5">' . $faq[$k]['rack'] . '</td>
					<td class="">' . $faq[$k]['shot_count'] . '</td>
					<td class="td10">' . $faq[$k]['max_shot_count'] . '</td>
					<td class="td12">' . $faq[$k]['vendor'] . '</td>
					<td class="td9">' . $faq[$k]['line'] . '</td>
					<td class="td13">' . $faq[$k]['remarks'] . '</td>
