...
if($_POST['id']){
	$id=$_POST['id'];
	$result = $conn->query('SELECT * FROM chipmount_nozzle WHERE package LIKE "%'.$id.'%" OR box_no LIKE "%'.$id.'%" OR nozzle_partno LIKE "%'.$id.'%" OR machine_model LIKE "%'.$id.'%" OR machine_id LIKE "%'.$id.'%" OR package LIKE "%'.$id.'%"' );
