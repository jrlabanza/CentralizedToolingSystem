...
if(empty($fileName)){
    ...
    $conn->query("UPDATE collet set status='".$status."',machine_id='".$machineID."',shots='".$shotCnt."',remarks='".$remarks."',loc='".$loc."' WHERE box_no = '".$boxNo."'");

    $conn->query("INSERT INTO collet_history(box_no, package, nozzle_partno, altrntv_nozzle, line, machine_model, machine_id, out_count, status, loc, max_shots, shots, remarks, client, clerk) 
            SELECT box_no, package, nozzle_partno, altrntv_nozzle, line, machine_model, machine_id, out_count, status, loc, max_shots, shots, remarks,'$borrower','".$_SESSION['userEmail']."' FROM collet WHERE box_no = '".$boxNo."'");
