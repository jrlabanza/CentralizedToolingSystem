...
if(empty($fileName)){
    $fileName = '';
            ...
            //$clerk = $_POST['clerk'];
            //$db = Db::getInstance();
            $conn->query("UPDATE center_board set `status`='".$status."',tester_id='".$tstID."',handler_id='".$hdID."', loc='".$loc."', remarks='".$remarks."', last_update='".$lastUpdate."' WHERE serial_id = '".$srid."'");
            
            $conn->query("INSERT INTO center_board_history(serial_id,type,family,tst_pf,status,tester_id,handler_id,loc,storage,line,vendor,borrower,clerk,sfile,remarks)
