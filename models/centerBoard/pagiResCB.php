...
if (empty($_SESSION['line'])){
	$sql = "SELECT * FROM center_board ORDER BY last_update DESC";
}else{
	$sql = "SELECT * FROM center_board WHERE line = '$_SESSION[line]' ORDER BY last_update DESC";
}

$paginationlink = "models/centerBoard/pagiResCB.php?page=";
...
if(!empty($perpageresult)) {
	$output = '<nav aria-label="..."><ul class="pagination">' . $perpageresult . '</ul><button type="button" class="btn btn-link" onclick=location.href="models/centerBoard/exportInventory.php">Download Inventory</button></nav>';
	$output .= '';
}else{
	$output = '<button type="button" class="btn btn-link" onclick=location.href="models/centerBoard/exportInventory.php">Download Inventory</button>';
}
