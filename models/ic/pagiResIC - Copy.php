...
if (empty($_SESSION['line'])){
	$sql = "SELECT * from ic ORDER BY last_update DESC";
}else{
	$sql = "SELECT * FROM ic WHERE line = '$_SESSION[line]' ORDER BY last_update DESC";
}

$paginationlink = "models/ic/pagiResIC.php?page=";
...
if(!empty($perpageresult)) {
	$output = '<nav aria-label="..."><ul class="pagination">' . $perpageresult . '</ul><button type="button" class="btn btn-link" onclick=location.href="models/ic/exportInventory.php">Download Inventory</button></nav>';
	$output .= '';
}else{
	$output = '';
}
...
$output .= '<table id="keywords" class="table table-bordered table-sm-responsive table-hover">
	...
	if ($_GET["rowcount"] > 0){
		foreach($faq as $k=>$v) {
			...
			$interval = new DateTime($faq[$k]['last_update']);
			...
			//$dateDif = $dateToday->diff($lastUpdate);//;
			...
			$output .= '
				<tr '.$style.'>
					...
					<td class="ven">' . $faq[$k]['dra'] . '</td>
					<td class="pinType">' . $faq[$k]['insert_sn'] . '</td>
					<td class="prtNo">' . $faq[$k]['clamp_sn'] . '</td>
					<td class="loc">' . $faq[$k]['loc'] . '</td>
					<td class="pinCount">' . $faq[$k]['out_count'] . '</td>
