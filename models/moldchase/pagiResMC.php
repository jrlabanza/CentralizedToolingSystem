...
if (empty($_SESSION['line'])){
	$sql = "SELECT * from mc ORDER BY last_update DESC";
}else{
	$sql = "SELECT * FROM mc WHERE line = '$_SESSION[line]' ORDER BY last_update DESC";
}

$paginationlink = "models/moldchase/pagiResMC.php?page=";
...
$output .= '<table id="keywords" class="table table-bordered table-sm-responsive table-hover">
	<thead id="thead" class="thead-light">
		<tr>
			<th scope="col" class="col-sm-0">MOLD CHASE ID</th>	
		...
		foreach($faq as $k=>$v) {
			$output .= '
					<tr style="cursor: pointer;">
						...
						<td class="td3">' . $faq[$k]['mold_die_id'] . '</td>
