...
if (!empty($filter[0]) && !empty($filter[1]) && !empty($filter[2])) {
	$result = $conn->query('SELECT * FROM mc_history WHERE status LIKE "%'.$filter[0].'%" OR serial_id LIKE "%'.$filter[0].'%" OR package LIKE "%'.$filter[0].'%" AND date_time between "'.$from.'" AND "'.$to.'" ORDER BY date_time DESC');
}elseif (!empty($filter[1]) && !empty($filter[2])) {
	$result = $conn->query('SELECT * FROM mc_history WHERE date_time between "'.$from.' 00:00:00" AND "'.$to.' 23:59:59" ORDER BY date_time DESC');
}elseif (!empty($filter[1])){
	$result = $conn->query('SELECT * FROM mc_history WHERE date_time >= "'.$from.'" ORDER BY date_time DESC');
}else {
	$result = $conn->query('SELECT * FROM mc_history WHERE status LIKE "%'.$filter[0].'%" OR serial_id LIKE "%'.$filter[0].'%" OR package LIKE "%'.$filter[0].'%" ORDER BY date_time DESC');
}
...
echo '<table id="keywords" class="table table-bordered table-sm-responsive table-hover">
	...
	while ($row = $result->fetch_assoc()) {
		echo '<tr>
			...
			<td class="stats">' . $row['mold_die_id'] . '</td>
